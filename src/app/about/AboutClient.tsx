...
interface Props {
  ...
  imgBlockIds: Record<string, string>
}

export default function AboutClient({ content, isEditorMode, imgBlockIds }: Props) {
      ...
      <section className="relative min-h-[60vh] flex items-center overflow-hidden bg-green-800">
        ...
        <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 py-24 text-center">
          ...
          <h1 data-cms-key="about.hero.headline" className="font-serif text-5xl md:text-6xl font-bold text-white mb-6">
            {c(content, 'about.hero.headline', `${t('about.title')} — ${t('about.subtitle')}`, lang)}
          </h1>
          <p data-cms-key="about.hero.tagline" className="text-xl text-white/80 max-w-2xl mx-auto leading-relaxed">
            {c(content, 'about.hero.tagline', 'Every garden deserves an advocate before a bulldozer.', lang)}
          </p>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
            ...
            <div>
              ...
              <h2 data-cms-key="about.story.title" className="font-serif text-4xl font-bold text-green-800 mb-6 leading-tight">
                {c(content, 'about.story.title', 'Rescue First. Plant Second.', lang)}
              </h2>

              <div className="space-y-4 text-gray-600 leading-relaxed">
                <p data-cms-key="about.bio.paragraph1">
                  {c(content, 'about.bio.paragraph1', lang === 'es' ? t('about.bio') : "Maytee founded Maytee's Garden Center with a mission that sets her apart from every other landscaper in South Florida: she evaluates before she removes. While most contractors arrive with shovels, Maytee arrives with a clipboard — cataloging what's already growing, what can be saved, and what truly needs to go.", lang)}
                </p>
                <p data-cms-key="about.bio.paragraph2">
                  {c(content, 'about.bio.paragraph2', lang === 'es' ? '' : 'This philosophy earned her the nickname "the lawyer of the plants" among her growing community of clients across Coral Gables, Pinecrest, Kendall, Doral, and greater Miami. Her approach consistently saves clients money, preserves mature specimens that take years to grow, and results in gardens that feel deeply rooted in their environment — because they are.', lang)}
                </p>
                {lang !== 'es' && (
                  <p>
              ...
              <blockquote className="mt-8 border-l-4 border-terra-500 pl-5 py-2">
                <div className="flex items-start gap-2">
                  ...
                  <p data-cms-key="about.bio.quote" className="font-serif text-xl text-green-800 italic leading-relaxed">
                    &ldquo;{c(content, 'about.bio.quote', 'Every plant has a story. My job is to make sure it gets a chance to finish telling it.', lang)}&rdquo;
                  </p>
                </div>
      ...
      <section className="py-12 bg-green-800">
        <div className="max-w-5xl mx-auto px-4">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-6 text-center text-white">
            <div>
              <div data-cms-key="about.stats.gardens" className="font-serif text-4xl font-bold text-green-300">{c(content, 'about.stats.gardens', '500+', lang)}</div>
              <div className="text-green-200 text-sm mt-1">Gardens Transformed</div>
            </div>
            <div>
              <div data-cms-key="about.stats.years" className="font-serif text-4xl font-bold text-green-300">{c(content, 'about.stats.years', '15+', lang)}</div>
              <div className="text-green-200 text-sm mt-1">Years of Expertise</div>
            </div>
            <div>
              <div data-cms-key="about.stats.reviews" className="font-serif text-4xl font-bold text-green-300">{c(content, 'about.stats.reviews', '200+', lang)}</div>
              <div className="text-green-200 text-sm mt-1">5-Star Reviews</div>
            </div>
            <div>
              <div data-cms-key="about.stats.rating" className="font-serif text-4xl font-bold text-green-300">{c(content, 'about.stats.rating', '4.9★', lang)}</div>
      ...
      <section className="py-16 bg-white">
        <div className="max-w-4xl mx-auto px-4 text-center">
          <h2 data-cms-key="about.visit.headline" className="section-title">
            {c(content, 'about.visit.headline', 'Come Visit Us', lang)}
          </h2>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-8">
            ...
            <div className="bg-green-50 rounded-2xl p-6">
              ...
              <p data-cms-key="about.visit.hours" className="text-gray-600 text-sm whitespace-pre-line">{c(content, 'about.visit.hours', 'Mon–Sun 9 AM – 5:30 PM (Fri–Sat until 6 PM)', lang)}</p>
            </div>
            <div className="bg-green-50 rounded-2xl p-6">
              ...
              <p data-cms-key="about.visit.contact" className="text-gray-600 text-sm whitespace-pre-line">
                {c(content, 'about.visit.contact', '(786) 227-6616\ninfo@mayteesgardencenter.com', lang)}
              </p>
            </div>
          </div>
          <div className="mt-10 flex flex-col sm:flex-row gap-4 justify-center">
            <Link href="/booking" className="btn-primary">{t('booking.title')} <ArrowRight className="w-4 h-4" /></Link>
            <Link href="/contact" className="btn-secondary">{t('contact.submit')}</Link>
          </div>
        </div>
      </section>
